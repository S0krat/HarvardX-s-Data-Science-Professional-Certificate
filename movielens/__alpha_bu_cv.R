...
rnames <- train_set$userId
train_set <- as.matrix(train_set[,-1])

rm(edx, val_index)

fit_users <- data.frame(userId = as.integer(rnames))
...
train_set <- sweep(train_set - mu, 2, b_i)

sums <- rowSums(train_set, na.rm = TRUE)
ns <- rowSums(!is.na(train_set))

rmses <- sapply(alphas, function(a){
  b_u <- sums / (ns + a)
  fit_users$b_u <- b_u
  inner_join(validate_set, fit_movies, by = "movieId") %>% 
    inner_join(fit_users, by="userId") %>%
